...
const db = new sqlite3_1.default.Database('voting_app.db');
exports.db = db;
    ...
    db.serialize(() => {
            ...
            if (err) {
                ...
                newColumns.forEach(column => {
                    ...
                    db.run(`PRAGMA table_info(users)`, (err) => {
                        ...
                        db.get(`SELECT name FROM pragma_table_info('users') WHERE name = ?`, [columnName], (err, row) => {
                            if (err) {
        ...
        `, (err) => {
            if (err) {
                ...
                db.get(`SELECT name FROM pragma_table_info('candidates') WHERE name = 'age'`, (err, row) => {
                    if (err) {
                    ...
                    if (row) {
                        ...
                        db.get(`SELECT name FROM pragma_table_info('candidates') WHERE name = 'date_of_birth'`, (err, dobRow) => {
                            if (err) {
                                console.error('Error checking for date_of_birth column:', err);
                                return;
                            }
                            if (!dobRow) {
                                db.run(`ALTER TABLE candidates ADD COLUMN date_of_birth TEXT NULL`, (err) => {
                                    if (err) {
                                        ...
                                        `, (err) => {
                                            ...
                                            `, (err) => {
                                                ...
                                                // Drop old table
                                                db.run('DROP TABLE candidates', (err) => {
                                                    if (err) {
                                                        console.error('Error dropping old candidates table:', err);
                                                        return;
                                                    }
                                                    // Rename new table
                                                    db.run('ALTER TABLE candidates_new RENAME TO candidates', (err) => {
                                                        if (err) {
