...
router.get('/id-proof/:filename', (req, res) => {
    ...
    const filePath = path_1.default.join(ID_PROOF_UPLOAD_DIR, filename);
...
router.post('/publish-results', (req, res) => {
    database_1.db.run('UPDATE election_state SET results_published = TRUE WHERE id = 1', (err) => {
        if (err) {
...
router.post('/candidates', (req, res) => {
    ...
    database_1.db.get('SELECT id FROM candidates WHERE name = ?', [name], (err, row) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Internal server error' });
        }
        ...
        database_1.db.run('INSERT INTO candidates (name, date_of_birth, party, image_url) VALUES (?, ?, ?, ?)', [name, date_of_birth || null, party || null, image_url || null], function (err) {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ error: 'Internal server error' });
            }
...
router.put('/candidates/:id', (req, res) => {
    ...
    database_1.db.get('SELECT id FROM candidates WHERE id = ?', [candidateId], (err, row) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Internal server error' });
        }
        ...
        if (name) {
            database_1.db.get('SELECT id FROM candidates WHERE name = ? AND id != ?', [name, candidateId], (err, existing) => {
                if (err) {
                    console.error('Database error:', err);
                    return res.status(500).json({ error: 'Internal server error' });
                }
    ...
    function updateCandidate() {
        ...
        database_1.db.run(`UPDATE candidates SET ${updates.join(', ')} WHERE id = ?`, values, function (err) {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ error: 'Internal server error' });
            }
            ...
            database_1.db.get('SELECT id, name, date_of_birth, party, image_url FROM candidates WHERE id = ?', [candidateId], (err, candidate) => {
                if (err) {
                    console.error('Database error:', err);
                    return res.status(500).json({ error: 'Internal server error' });
                }
...
router.delete('/candidates/:id', (req, res) => {
    ...
    database_1.db.get('SELECT id FROM candidates WHERE id = ?', [candidateId], (err, row) => {
        ...
        // Delete candidate
        database_1.db.run('DELETE FROM candidates WHERE id = ?', [candidateId], function (err) {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ error: 'Internal server error' });
            }
...
router.get('/users', (req, res) => {
    database_1.db.all('SELECT id, username, has_voted, is_admin FROM users', (err, rows) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Internal server error' });
        }
...
router.get('/users/pending', (req, res) => {
    database_1.db.all(`SELECT id, username, full_name, address, mobile_number, date_of_birth, id_proof_filename, face_descriptors 
         FROM users 
         WHERE application_status = 'Pending'`, (err, rows) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Internal server error' });
        }
...
router.post('/users/:id/approve', (req, res) => {
    ...
    database_1.db.get('SELECT id FROM users WHERE id = ?', [userId], (err, row) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Internal server error' });
        }
        ...
        database_1.db.run('UPDATE users SET application_status = ?, rejection_reason = NULL WHERE id = ?', ['Approved', userId], function (err) {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ error: 'Internal server error' });
            }
...
router.post('/users/:id/reject', (req, res) => {
    ...
    database_1.db.get('SELECT id FROM users WHERE id = ?', [userId], (err, row) => {
        ...
        database_1.db.run('UPDATE users SET application_status = ?, rejection_reason = ? WHERE id = ?', ['Rejected', reason || null, userId], function (err) {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ error: 'Internal server error' });
            }
